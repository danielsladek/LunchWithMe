...
import {
  ...
  EVENTS_FEED_FAIL,
  EVENTS_FEED_FETCH,
} from '../../pages/EventFeedPage/Actions'
import Api from '../../Api'
...
function* switchAttendance(action) {
  try {
    const api = new Api(),
      ...
      userHasAttendanceRecord = action.payload.event.eventAttendees.find(
        attendee => attendee.id == loggedUser.userId
      ),
    ...
    const switchAttendanceInDb = yield api.changeAttendance(
      eventId,
      userId,
      willAttend
    )
    ...
    if (typeof userHasAttendanceRecord == 'undefined') {
      ...
      changedEvent.eventAttendees.find(
        attendee => attendee.id == loggedUser.userId
...
function* deleteEvent(action) {
  try {
    ...
    /* Change event attendance in DB */
    const deleteEventInDb = yield api.deleteEvent(eventId)
