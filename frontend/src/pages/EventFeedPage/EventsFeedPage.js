      ...
      <div className="feed-wrapper">
        ...
        <Row className="eventsFeedPage">
          {(events.length &&
            events.map(event => {
              if (event) {
                let eventDate = event.timeStart.match(/([0-9]){4}-([0-9]){2}-([0-9]{2})/);
                ...
                return <EventPanel eventId={event.id} event={event} key={event.id} />;
              }
            })) || (
            <h2>
              There are currently no events. <Link to="addEvent">Let's create one!</Link>
            </h2>
          )}
        </Row>
      </div>
